    ...
    const NavigateToScore = () =>{
        ...
        navigateScore('/results', {
            state: {
                ...
                quizTitle:CssQuizTitle,
                quizIcon:CssQuizIcon } });
    }

    const CssData = quizData?.quizzes?.find(
        (item) => item.title === 'CSS' && item.icon
    );
    const CssQuizTitle = CssData?.title;
    const CssQuizIcon = CssData?.icon;
    const Questions = CssData?.questions || [];
                    ...
                    <div className="container">
                        <span className='accessibility-style theme-course-style'>
                            <div className="accessibility-img-wrapper theme-course-wrapper">
                                <img className='access-background' src={CssQuizIcon} alt="js-icon"/>
                                <h3 className={`subject-name ${isLightOn ? 'theme-subject-dark' : 'theme-subject-light'}`}>{CssQuizTitle}</h3>
                            </div>
